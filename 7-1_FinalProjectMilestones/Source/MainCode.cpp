...
int main(int argc, char* argv[])
{
	...
	const std::vector<std::pair<const char*, const char*>> shaderPaths =
	{
		{ "Source/shaders/vertexShader.glsl", "Source/shaders/fragmentShader.glsl" },
		{ "../../Utilities/shaders/vertexShader.glsl", "../../Utilities/shaders/fragmentShader.glsl" }
	};

	for (const auto& shaderPath : shaderPaths)
	{
		shaderProgram = g_ShaderManager->LoadShaders(shaderPath.first, shaderPath.second);
		if (shaderProgram != 0)
		{
			break;
		}
